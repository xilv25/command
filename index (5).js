...
// key: guildId -> { reviewChannelId, reportChannelId }
...
async function registerSlashCommands() {
  ...
  const setupCommand = new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub
        .setName("channel")
        .setDescription("Set channel review & report")
        ...
        .addChannelOption(opt =>
          opt
            .setName("report")
            .setDescription("Channel untuk report (ex-suggest)")
...
client.on("interactionCreate", async interaction => {
    ...
    if (commandName === "setup") {
      ...
      if (sub === "channel") {
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: "Konfigurasi bot review & report" })
          .setTimestamp();
      ...
      if (sub === "command") {
        ...
        const name = nameRaw.toLowerCase().trim(); // contoh: eletrident
  ...
  if (interaction.isButton()) {
    ...
    // Tombol "Work" di DM -> kirim embed formulir + tombol "Isi formulir"
    if (type === "cmd_work") {
      const guildId = parts[1];
      const cmdName = parts[2];

      // cancel reminder kalau ada
    ...
    // Tombol di embed formulir: "Isi formulir"
    if (type === "review_open") {
      const guildId = parts[1];
      const cmdName = parts[2];
      ...
      // disable tombol "Isi formulir" di embed formulir
    ...
    // Tombol "Doesn't work" -> kirim embed backup
    if (type === "cmd_fail") {
      const guildId = parts[1];
      const cmdName = parts[2];
  ...
  if (interaction.isModalSubmit()) {
    ...
    // tandai user sudah pernah review command ini
    const key = `${guildId}:${cmdName}:${interaction.user.id}`;
    reviewDone.add(key);

    // Jika rating 1 -> blacklist + arahkan ke report
    if (rating === 1) {
      const blKey = `${guildId}:${interaction.user.id}`;
      blacklist.add(blKey);
...
// Helper buat potong text biar nggak kepanjangan di field
...
// Helper kirim DM kalau user di-blacklist
...
// ====== HANDLE PREFIX COMMAND & .clear ======
const PREFIX = "!";

client.on("messageCreate", async message => {
  if (message.author.bot) return;
  if (!message.guild) return; // cuma di server

  const contentLower = message.content.trim().toLowerCase();

  // ==== CLEAR COMMAND DI CHANNEL REPORT ====
  if (
  ...
  // DM user dengan embed tema #00cf91
  ...
  try {
    ...
    // === SETUP REMINDER 1 MENIT KALAU BELUM KLIK WORK ===
    ...
    // Pesan konfirmasi di server + countdown 3 → 2 → 1 → delete
    let remaining = 3;
